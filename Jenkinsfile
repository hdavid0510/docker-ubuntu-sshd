	...
	stages {
			...
			parallel {
				...
				stage('linux/386') {
					steps {
						...
						sh 'docker buildx build --platform linux/386 -t $IMAGE_NAME:$IMAGE_TAG-386 .'
					}
				}
			}
		}
		stage('Push multiarch image') {
			steps {
				...
				sh 'docker buildx build --push --platform linux/amd64,linux/arm/v7,linux/arm64,linux/ppc64le,linux/s390x,linux/386 -t $IMAGE_NAME:$IMAGE_TAG .'
			}
		}
	}
	post {
